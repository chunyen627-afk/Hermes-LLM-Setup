...
**開機自動啟動**：排程工作 `Qwen38-GPU-Server`（登入後延遲 1 分鐘）
呼叫 `gpu-host/START-ALL.bat`，一次帶起三個服務：

| 服務 | port | 說明 |
|---|---|---|
| llama-server | 8001 | 模型本體，走 `_bootmenu.ps1`（10 秒選單，預設韌體模式）|
| 橋接器 | 1234 | Hermes 和家人 GUI 都連這個 |
| 家人 GUI | 5000 | 手機/家人用，看圖走本機 27B |

每一項都先偵測，已經在跑就跳過，所以重複執行是安全的。
桌面捷徑「啟動 27B」跑的也是這支 —— **開機和手動同一條路**。

⚠ **檔名和路徑一律用 ASCII**。原本叫 `啟動-27B.bat`，
但中文檔名在排程 / PowerShell / cmd 之間會編碼壞掉，
`Set-ScheduledTask` 一直報「找不到檔案」，改成 `START-ALL.bat` 才成功。
桌面捷徑的顯示名稱不受影響，還是中文。
