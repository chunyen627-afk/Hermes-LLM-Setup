...
## ⚠ 統計數字會騙人（2026-08-28 實測教訓）

**不要用「亮點像素總數」當驗證標準。**

實例：dump 出畫面後數亮點，得到 11,596 個，跟預期元素
（球+拖尾+掃描線+點環）的估算吻合，於是判定「乾淨、沒毀損」。

實際看那張圖：**整片是黑色橫條紋，一個動畫元素都沒有**。
那些亮點全來自條紋，數量剛好對得上。

**「該亮變黑」和「該黑變亮」會互相抵消，總數看不出來。**

### 所以

1. **一定要真的用 vision 看那張圖**，不能只看統計
2. **問具體問題**，不要問「這張圖正常嗎」：
   - 背景是什麼顏色？（對照你預期的）
   - 有幾個圓/方塊？座標大概在哪？
   - **有沒有不該存在的東西**（條紋、雜點、色塊）？
   - 畫面上的文字是什麼？（HUD 格式壞掉很常見，例如
     `FPS 60` 變成 `FPS 0E 21171`）
3. **雙緩衝要兩塊都 dump 都看** —— 正常情況兩塊應該很相似
   （只差一幀）。連底色都不同就是嚴重的邏輯錯誤

### 檔案大小是個免費的線索

兩個緩衝區 dump 出來的 PNG 大小差很多（例如 5K vs 35K），
通常代表內容差異巨大 —— PNG 壓縮率反映了圖像複雜度。
**一塊 5K 一塊 35K，幾乎肯定有一塊是錯的。**
