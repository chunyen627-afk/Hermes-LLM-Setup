...
def check_spec(spec, base):
    """Confirm every item the spec document promises exists in the source.

    The gate proves a test ran and hit its paths. It cannot prove the design
    contains everything the architecture document says it does -- a register
    the spec defines but nobody implemented has no failing test, because no
    test was ever written for it. This closes that gap by grepping for each
    promised item.

    spec: {"<label>": {"pattern": "<regex>", "in": ["file", ...],
                       "why": "why this matters"}}
    Returns a list of failure strings (empty means everything was found).
    """
    missing = []
    for label in sorted(spec):
        ...
        if isinstance(files, str):
            files = [files]
        if not files:
            missing.append("spec item %r has no 'in' file list" % label)
            continue
        found = False
        for rel in files:
        ...
        if not found:
            why = item.get("why")
            missing.append("spec item '%s' not found in %s%s"
                           % (label, ", ".join(files),
                              " -- " + why if why else ""))
    return missing
...
def main():
    ...
    if a.list:
        ...
        for name in blocks:
            ...
            if b.get("spec"):
                print("%-18s   ^ spec: %d items must exist in source"
                      % ("", len(b["spec"])))
    ...
    for block_name, src_cfg in units:
        ...
        # Spec conformance: does the source contain everything the
        # architecture document promises? A register defined in the spec but
        # never implemented has no failing test -- nobody wrote one.
        spec = src_cfg.get("spec") or {}
        spec_missing = check_spec(spec, base) if spec else []
        ...
        for label, p in runs:
            ...
            if spec_missing:
                reasons = list(reasons) + spec_missing
                ok = False
                stats["spec_missing"] = len(spec_missing)
