...
**沒有真硬體時，這個問題沒有絕對答案。** 但有四個做法能把
「憑感覺覺得夠了」換成「有依據地相信它夠了」。按投報率排序：

**1. 從 datasheet 逐條對照，不是憑印象寫**

模仿的是現成規格（PSRAM、SD 卡、I2C 器件…）時，datasheet 的時序圖
和狀態表就是規格書。把每一條列成清單，逐條問「我的模型做了嗎」。

⚠ **最容易漏的是初始化流程** —— 大家都記得寫資料傳輸，
忘記真實主機開機時會先讀 ID、設 mode register、校準 latency。
**那些不回應，主機根本走不到資料傳輸那一步。**

**2. 把 datasheet 的限制寫成 assertion，讓模型自己抓自己**

不只「模型會做什麼」，還要「模型不准做什麼」：

```verilog
// datasheet 說 CS 拉低後至少要等 N 拍才能發第一個命令
if (cs_fall && cmd_start && (cycles_since_cs < N))
    $display("ASSERT tb_master_violates_tCSS");
// datasheet 說連續兩次存取之間有最短間隔
if (cs_fall && (cycles_since_last_cs_rise < T_MIN))
    $display("ASSERT tb_master_violates_tCPH");
```

**這些 assertion 保護的是「模型的真實性」**，不是 DUT。
模型自己違規時你會知道 —— 那代表它在做真實主機不會做的事，
用它測出來的結果不能信。

**3. 反過來測：故意讓模型做「規格允許但你沒想到」的事**

規格允許的行為空間比你寫的測試大。列出規格沒禁止的變化，
每一種都跑一次：

- 時脈頻率在允許範圍的兩端（不是只測中間值）
- 命令之間的間隔：最短、最長、隨機
- 突發長度：1 拍、最大、跨邊界
- 中途中止：每一個階段（命令中、位址中、dummy 中、資料中）都試

**規格允許的組合，真實主機遲早會做出來。**

**4. 用可觀察的行為當交叉驗證**

如果那個器件在真實系統裡有已知的行為特徵，拿來對照：
真實 PSRAM 在某個頻率下的 dummy cycles 是幾拍、
某個命令的回應延遲是多少 —— 模型算出來的數字對得上嗎？
...
```markdown
[ASSUMPTION] xSPI slave 只在模擬中對過 —— testbench 的 master
是照 <某某型號> datasheet rev X 寫的，沒有接過真的 STM32。
已對照的項目：初始化序列、讀寫命令、dummy cycles、突發遞增、
CS 中止。未驗證：實際時序裕度、訊號完整性、主機韌體的實際行為。
```

**「我照 datasheet 做了而且測了 N 項」跟「這東西能用」是兩句話。**
寫成前者，不要寫成後者 —— 接手的人才知道風險在哪。
