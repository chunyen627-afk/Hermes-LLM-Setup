...
**「動手前」是指寫第一行 code 之前，不是做到一半卡住才想起來。**
2026-08-30 的實例：一個硬體任務做了六小時、卡在精度問題出不來，
才讀到 `hardware-design-tradeoffs`（那份 skill 開頭就寫著
「動手前該回答的六個問題」，包含精度該怎麼定）。**早讀六小時就省下來了。**

特別是這幾種任務，**一定有對應的 skill，先讀再說**：
硬體 / RTL、韌體、驗證方法、做投影片、遊戲素材、
以及任何你「不太確定這台機器是怎麼設定的」的事。
...
**不要一路做到被強制中斷。** 2026-08-30 的實例：一個 AXI 加速器任務
連續跑 8.2 小時、254 次工具呼叫，中間撞兩次工具上限、壓縮四次 ——
使用者到最後才看到成果，中間八小時沒機會確認方向對不對。

**什麼是「一個可驗證的單元」**：做完之後你能拿出證據說「這塊是對的」。
例如：一個模組通過它的單元測試、一個功能端到端跑通、一個 bug 修好並複現不出來。
不是「寫完了」，是「證明它對了」。

**停下來的時候要給**：

1. 這個單元做了什麼、**怎麼證明它對**（實際的數字，不是「應該沒問題」）
2. 下一個單元打算做什麼
3. 有沒有卡住的地方、有沒有需要使用者決定的事

然後**真的停**，等使用者回應。不要自己接著做下一個。

**例外**：使用者明確說「一路做完不要停」、或單元小到分開報告很囉唆
（例如修三個獨立的小 typo）。

**卡住的判斷**：同一個問題試了 **5 次以上沒有進展**，就停下來說
「我卡在 X，試過 A/B/C 都不行，目前的假設是 Y」——
不要無限重試。使用者可能一句話就解開你卡兩小時的東西。

**投報率的判斷**：如果你發現自己在「把已經夠好的東西做得更好」
（追求更高精度、更快、更省），先問**「不做會怎樣？」**——
答不出具體損失就別做，那是在雕不重要的地方。
