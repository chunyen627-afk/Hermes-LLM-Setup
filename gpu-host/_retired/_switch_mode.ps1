...
# ⚠ 2026-08-29 起預設就是 share（2 slot），solo 只在特殊情況才用。
# 原因：視覺請求（vision_analyze）會另外開一個請求，1 slot 時它得排隊等
# 主任務跑完 -> 必定 timeout（實測 900 秒都等不到）。2 slot 下 21.9 秒完成。
# 只要還想讓模型自己看圖，就不要切回 solo。
$MODES = @{
    solo  = @{ Slots = 1; Ctx = 204800; Desc = '獨佔 — 1 slot，ctx 200K（⚠ 視覺會 timeout）' }
    share = @{ Slots = 2; Ctx = 245760; Desc = '共享 — 2 slot 各 120K，視覺可用（預設）' }
}
...
if (-not $Mode) {
    ...
    Write-Host '       ⚠ 模型看圖會 timeout（視覺請求得排隊等主任務）' -ForegroundColor DarkYellow
    Write-Host '       只在確定不看圖、又需要超長 ctx 時才選。' -ForegroundColor DarkGray
    Write-Host ''
    Write-Host '   [2] 共享 — 2 slot 各 120K（預設，建議）' -ForegroundColor Green
    Write-Host '       模型看得到圖，你和家人同時用也不互搶。' -ForegroundColor DarkGray
    Write-Host ''
