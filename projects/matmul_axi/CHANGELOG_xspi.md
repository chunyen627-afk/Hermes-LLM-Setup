...
## 💡 01:00 —— 你來回跳三次的根本原因：一個訊號管了兩件事

回顧今天的軌跡，你在同一個問題上來回改了三輪：

| 版本 | 擋掉垃圾筆 | hwleft 算對 |
|---|---|---|
| `wr_data_started`（posedge 設定）| ✅ | ❌ 算成 0 → 死結 |
| `pipe_loaded`（negedge 設定）| ❌ 垃圾回來 | ✅ 引擎能動 |

**兩個都不是錯的做法 —— 錯的是把兩件事綁在同一個訊號上。**

- 「擋掉每個 frame 第一次 stale commit」需要 **posedge 就為真**的閘門
- 「`wr_hw_cnt` 數對」需要 **只數有效 commit** 的條件

這是兩個不同的需求，用同一個訊號一定顧此失彼。**拆成兩個就同時成立。**

### 具體要做的

```verilog
// 兩個各自獨立的訊號，不要共用
// (1) 擋 commit：posedge 設定，擋得掉第一次 stale
// (2) 數 hwleft：只在真正有效的 commit 時 +1
```

自己決定怎麼命名和實作，但**確認它們是兩個獨立訊號**。

### 這是通則，不只這一題

**以後發現自己在兩個版本之間反覆改、每次都只對一半 —— 先停下來問：
是不是兩件事被綁在同一個訊號上了？** 那時候不該繼續二選一，
該拆開。（這條已經寫進派工模板第 10 條。）
...
## 🎉 01:30 —— 主指標 28 小時來第一次移動：26 → 25

```
CHECK data_integrity 26 25      ← 從 08-31 20:37 卡到現在，終於降了
AXI DDR aw=13 w=17 b=13         ← 寫入端維持
```

**規劃者已驗證這是真的進展，不是測試被放寬**：
tb 驗收邏輯（`chk_bad =` / `chk_checked =` / `CHECK` 那三處）的
md5 指紋跟基準完全一致 —— `b8d20766db2b0402f81bc1f97de69750`，沒被動過。

### 而且讀取路徑露出了關鍵訊號

```
WRITE_VERIFY hw 0: got 0000 expected 00ff
WRITE_VERIFY hw 1: got 00ff expected 01fe    ← 注意！不是 xxxx 也不是 0000
```

**`hw 1` 讀回 `00ff`，那正是 `hw 0` 的期望值 —— 讀取整串晚了一格。**

這跟你解寫入端時的症狀**完全同型**（當初是 `fc5b 0258 0359...`，
每筆是下一筆的值）。你已經解過一次這個型態的問題，用同一套方法：

1. 用 `RDWR` / `RCOMMIT` 逐拍印出讀取鏈每一級的值
2. 跟期望序列並排比對，看是哪一級開始偏移
3. 一次只改一級，改完立刻重跑看往哪個方向動

### 特別注意（你解寫入端時踩過的兩個坑）

- **同一個 always block 裡用 NBA 讀自己正在賦值的訊號 → 拿到舊值**
  （`hw_pipe_lo <= w_lo` 那次）。讀取端如果有類似寫法，會造成一樣的偏移。
- **相位進入時機差一拍**（`dummy_n - 2` 那次）。讀取端也有相位問題的話，
  先確認第一筆是在哪個 edge 被推上線的。

### 進度

| 路徑 | 狀態 |
|---|---|
| 寫入：halfword 組裝 → beat → DDR 模型 | ✅ 完成，已凍結 |
...
## ⛔ 13:10 —— 你把讀取的 `-3` 改回 `-2` 了，這是第五次來回（規劃者收行程重派）

規劃者實測 13:07：`dummy_cnt` 只剩一行 `-2`（`is_read` 分支被拿掉），
沒有加任何首筆閘門。錯誤型態退回「交替垃圾」：

```
-3 時：hw0=xxxx  hw1=00ff  hw2=01fe  hw3=02fd   ← 資料全對、整串晚一格（較好）
-2 時：hw0=0000  hw1=00ff  hw2=0000  hw3=xxxx   ← 交替垃圾（現在）
```

你截斷前的推理是「delay P_DATA entry by one cycle for reads」——
那就是往 `-2` 走回去。**兩個 dummy 值你都試過了，都不會單獨成立。**
這跟寫入端 `wr_data_started` / `pipe_loaded` 來回四次一模一樣：
**不是選哪個值，是少了第二個訊號。**
